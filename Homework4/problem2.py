...
def compute_dL_da2(a2, y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_da2 = np.matrix(np.zeros((a2.shape[0], 1)))
    if a2[y] == 0:
        dL_da2[y] = -1000000
    else:
        dL_da2[y] = -1/a2[y]
...
def compute_da2_dz2(a2):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    da2_dz2 = np.zeros([a2.shape[0], a2.shape[0]])
...
def compute_dz2_dW2(a1,c):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz2_dW2 = np.zeros([c, a1.shape[0]])
    for i in range(c):
        if i == 0:
            dz2_dW2 = a1.transpose()
        else:
            dz2_dW2 = np.concatenate((dz2_dW2, a1.transpose()), axis=0)
    dz2_dW2 = np.matrix(dz2_dW2)
...
def compute_gradients(dL_da2, da2_dz2, dz2_dW2, dz2_db2, dz2_da1, da1_dz1, dz1_dW1, dz1_db1):
    ...
    ## INSERT YOUR CODE HERE
    # the 2nd layer
    dL_dz = sr.compute_dL_dz(dL_da2, da2_dz2)
    dL_dW2 = sr.compute_dL_dW(dL_dz, dz2_dW2)
    dL_db2 = sr.compute_dL_db(dL_dz, dz2_db2)

    # the 1st layer
    dL_da1 = compute_dL_da1(dL_dz, dz2_da1)
    dL1_dz1 = compute_dL_dz1(dL_da1, da1_dz1)
    dL_dW1 = sr.compute_dL_dW(dL1_dz1, dz1_dW1)
    dL_db1 = sr.compute_dL_db(dL1_dz1, dz1_db1)
